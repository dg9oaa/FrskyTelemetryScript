...
function mapLib.gmapcatcher_tiles_to_path(tile_x, tile_y, level)
  -- The division produced a decimal number, which was then rounded up. Rounding up should not occur. Hence the change
...
function mapLib.drawTiles(width,xmin,xmax,ymin,ymax,color,level)
  ...
  lcd.drawFilledRectangle(3, y-40, x+5+scaleLen, 30)
  --libs.drawLib.drawBitmap(5, y-15, "maps_box_380x20") --160x90
...
function mapLib.drawMap(widget, x, y, w, h, level, tiles_x, tiles_y, heading)
  ...
  if zoomUpdate == true then
    lcd.color(WHITE)
    lcd.font(FONT_XL)
    lcd.drawText(800/2, 480/2 - 15, string.format("ZOOM %d", level), CENTERED)
